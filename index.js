...
async function renderProducts(query)
{
    ...
    if(userInfo.isAdmin === true)
    {
        const rawData = await loadProducts()
        for(let i = 0; i < data.length; i++)
        {
            data[i] = {...data[i],priceUSD: rawData[i].price}
        }

        categories = await loadCategories()

        let addProductCard = document.createElement("div")
    ...
    shopItems.forEach(shopItem =>
        {
            ...
            cartItemsContainer.appendChild(documentFragment);
